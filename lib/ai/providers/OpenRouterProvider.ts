...
Use exact header names. null if no match. Recognize synonyms. Ignore UTM/click IDs/internal IDs.`;
      ...
      this.markSuccess();
      logger.info('OpenRouter schema inference complete', {
      ...
      logger.error('OpenRouter schema inference failed', {
        durationMs: Date.now() - startTime,
        error: message,
      });
      throw error;
...
Preserve original values as keys. Empty string if unclear.`;
      ...
      this.markSuccess();
      logger.info('OpenRouter semantic inference complete', {
        durationMs: Date.now() - startTime,
      ...
      const message = error instanceof Error ? error.message : String(error);
      this.markError(message);
      logger.error('OpenRouter semantic inference failed', {
        durationMs: Date.now() - startTime,
        error: message,
      });
      throw error;
  ...
  async inferSemanticMapping(
    ...
    try {
      ...
      this.markSuccess();
      logger.info('OpenRouter semantic mapping complete', {
        durationMs: Date.now() - startTime,
      ...
      const message = error instanceof Error ? error.message : String(error);
      this.markError(message);
      logger.error('OpenRouter semantic mapping failed', {
        durationMs: Date.now() - startTime,
        error: message,
      });
      throw error;
